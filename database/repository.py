...
class Database:
    ...
    async def update_user(self, user: User):
        await self._conn.execute('''
            UPDATE users SET trust = ?, mood = ?, message_count = ?, xp = ?, coins = ?, is_banned = ?, last_daily_time = ?, username = ?, custom_prompt = ?, is_vip = ?, profile_frame = ?, tutorial_done = ?, streak_count = ?, last_streak_date = ?, referred_by = ? WHERE id = ?
        ''', (user.trust, user.mood, user.message_count, user.xp, user.coins, user.is_banned, user.last_daily_time, user.username, user.custom_prompt, user.is_vip, user.profile_frame, user.tutorial_done, user.streak_count, user.last_streak_date, user.referred_by, user.id))
        await self._conn.commit()
        
    async def get_all_users(self) -> List[User]:
        async with self._conn.execute('SELECT id, trust, mood, message_count, xp, coins, is_banned, last_daily_time, username, custom_prompt, is_vip, profile_frame, tutorial_done, streak_count, last_streak_date, referred_by FROM users') as cursor:
    ...
    async def get_top_users_by_xp(self, limit: int = 10) -> List[User]:
        async with self._conn.execute('SELECT id, trust, mood, message_count, xp, coins, is_banned, last_daily_time, username, custom_prompt, is_vip, profile_frame, tutorial_done, streak_count, last_streak_date, referred_by FROM users ORDER BY xp DESC LIMIT ?', (limit,)) as cursor:
            rows = await cursor.fetchall()
            return [User(*row) for row in rows]
